...
export function activate(context: vscode.ExtensionContext) {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('mmrDev.useSelection', () => {
			const editor = vscode.window.activeTextEditor;
			if (!editor) { vscode.window.showInformationMessage('No active editor found.'); return; }
			const selectedText = editor.document.getText(editor.selection);
			if (!selectedText.trim()) { vscode.window.showInformationMessage('No code selected. Highlight code first.'); return; }
...
class SidebarProvider implements vscode.WebviewViewProvider {
	...
	private _lastAnalysis: string = '';
	private _lastInput: string = '';
	private _mode: Mode = 'feature';
	...
	private getWorkspaceRoot(): string {
		...
		const mmr = folders.find(f =>
			f.name.toLowerCase().includes('mmr') ||
	...
	public resolveWebviewView(webviewView: vscode.WebviewView) {
		...
		webviewView.onDidChangeVisibility(() => {
			if (webviewView.visible) this.tryInjectCurrentSelection();
		});

		webviewView.webview.onDidReceiveMessage(async (data) => {
			if (data.command === 'ask') await this.handleMessage(data.text, data.mode as Mode);
			if (data.command === 'set_mode') {
				this._mode = data.mode as Mode;
				this._state = 'idle';
				this._lastAnalysis = '';
				this._lastInput = '';
			}
			if (data.command === 'get_selection') this.tryInjectCurrentSelection();
		});
	}

	private tryInjectCurrentSelection() {
		const editor = vscode.window.activeTextEditor;
		if (!editor) return;
		const selectedText = editor.document.getText(editor.selection);
		if (selectedText.trim()) this._view?.webview.postMessage({ command: 'inject_code', code: selectedText });
	}

	private async handleMessage(text: string, mode?: Mode) {
		...
		if (this._state === 'idle') {
			...
			try {
				...
				if (this._mode === 'feature' || this._mode === 'refactor') {
					...
					setTimeout(() => this.sendAssistantMessage(
						'💬 Would you like me to apply these changes to the relevant files in your workspace?\n\nType **yes** to proceed or **no** to skip.',
						'confirm'
					), 400);
				}
			} catch {
			...
			if (answer === 'yes' || answer === 'y') {
				const loadingId = this.sendLoading();
				try {
					const response = await axios.post('http://127.0.0.1:8000/apply_changes', {
						story: this._lastInput, analysis: this._lastAnalysis, workspace_root: workspaceRoot
					});
					const changes: Array<{ file: string; content: string }> = response.data.changes || [];
					await this.applyFileChanges(changes);
					this.resolveLoading(loadingId, `✅ Changes applied to:\n${changes.map(c => `- \`${c.file}\``).join('\n') || 'No files returned.'}`);
				} catch { this.resolveLoading(loadingId, '⚠️ Could not apply changes. Backend error.'); }
			}
			setTimeout(() => this.sendAssistantMessage(
				'🧪 Would you like me to generate and add **RSpec test cases** for the affected code?\n\nType **yes** to proceed or **no** to finish.',
				'confirm'
			), 400);
			...
			if (answer === 'yes' || answer === 'y') {
				const loadingId = this.sendLoading();
				try {
					const response = await axios.post('http://127.0.0.1:8000/generate_tests', {
						story: this._lastInput, analysis: this._lastAnalysis, workspace_root: workspaceRoot
					});
					const tests: Array<{ file: string; content: string }> = response.data.tests || [];
				...
				} catch { this.resolveLoading(loadingId, '⚠️ Could not generate tests. Backend error.'); }
			} else {
				this.sendAssistantMessage('👍 All done! Select a mode below to start a new task.', 'assistant');
			}
			this._state = 'idle'; this._lastInput = ''; this._lastAnalysis = '';
			...
			} catch { this.resolveLoading(loadingId, '⚠️ Backend not running.'); }
		}
	}

	private async applyFileChanges(changes: Array<{ file: string; content: string }>) {
		const workspaceFolders = vscode.workspace.workspaceFolders;
		if (!workspaceFolders || changes.length === 0) return;
	...
	private sendLoading(): string {
		const id = Date.now().toString();
		this._view?.webview.postMessage({ command: 'loading', id });
		return id;
	}
	private resolveLoading(id: string, text: string) { this._view?.webview.postMessage({ command: 'resolve', id, text }); }
	private sendAssistantMessage(text: string, type: string) { this._view?.webview.postMessage({ command: 'assistant_message', text, type }); }
